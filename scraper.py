#!/usr/bin/env python3
"""Fetch CUPL Civil, Commercial and Economic Law School notices and keep history."""
...
BASE_URL = "https://msjjfxy.cupl.edu.cn"
USER_AGENT = "Mozilla/5.0 (compatible; cupl-msjjfxy-notice-watch/1.0; +https://github.com/houdemingfagewuzhigong)"
DATA_DIR = Path("data")

SECTIONS = [
    {"name": "通知公告", "path": "xwygg/tzgg.htm", "fallback": "home-notice"},
    {"name": "学院新闻", "path": "xwygg/xyxw.htm"},
    {"name": "学术动态", "path": "xsyj/xsdt.htm"},
]
...
def safe_open(url: str) -> str:
    try:
        return open_with_challenge(url)
    except Exception:
        return ""
...
def detail_date(url: str) -> tuple[str, str]:
    body = safe_open(url)
    text = clean(body)
...
def parse_cards(html_text: str, section: str, source_url: str) -> list[Notice]:
    seen_at = now_iso()
    items: list[Notice] = []
    pattern = re.compile(
        r'<DIV\s+class="cate-titlelist clearfix">\s*<A\s+href="([^"]+)">\s*<H4>(.*?)</H4>\s*</A>\s*<P>(.*?)</P>\s*(?:<P>(.*?)</P>)?',
        re.S | re.I,
    )
    for href, title_html, date_or_summary, summary_html in pattern.findall(html_text):
        url = absolute_url(href, source_url)
        title = clean(title_html)
        date = normalize_date(date_or_summary)
        summary = clean(summary_html or ("" if date else date_or_summary))
        if not date:
            detail_seen_date, detail_summary = detail_date(url)
            date = detail_seen_date
            summary = summary or detail_summary
        items.append(Notice(notice_id(url), title, date, url, summary, section, source_url, seen_at, seen_at))
    return items


def parse_home_notices(html_text: str, source_url: str) -> list[Notice]:
    block = re.search(r'<ul>\s*(<li><a href="info/1045/.*?</ul>)', html_text, re.S | re.I)
    if not block:
        return []
    seen_at = now_iso()
    items: list[Notice] = []
    for href, title_html in re.findall(r'<li><a href="([^"]+)">(.*?)</a>', block.group(1), re.S | re.I):
        url = absolute_url(href, source_url)
        date, summary = detail_date(url)
        items.append(Notice(notice_id(url), clean(title_html), date, url, summary, "通知公告", source_url, seen_at, seen_at))
    return items
...
def fetch(max_pages_per_section: int = 2) -> list[Notice]:
    ...
    for section in SECTIONS:
        ...
        first_html = safe_open(first_url)
        if not first_html and section.get("fallback") == "home-notice":
            notices.extend(parse_home_notices(open_with_challenge(BASE_URL + "/"), BASE_URL + "/"))
            continue
        pages = section_pages(section["path"], first_html, max_pages_per_section)
        for page_path in pages:
            page_url = urllib.parse.urljoin(BASE_URL + "/", page_path)
            page_html = first_html if page_path == section["path"] else safe_open(page_url)
            notices.extend(parse_cards(page_html, section["name"], page_url))
...
def save(notices: list[Notice]) -> None:
    ...
    meta = {
        "site": "中国政法大学民商经济法学院",
        "base_url": BASE_URL,
        "notice_url": BASE_URL + "/xwygg/tzgg.htm",
        "updated_at": seen_at,
